...
if (isset($_POST['create_review'])) {
  ...
  if (!empty($restaurantName) && !empty($productName) && !empty($email) && !empty($rating) && !empty($review)) {
    ...
    header('Location: ../views/review/createReview.php?msg=success');
  } else {
    header('Location: ../views/review/createReview.php?err=empty_fields');
  }
}
...
if (isset($_GET['action']) && $_GET['action'] == 'view_reviews') {
    ...
    require_once '../views/review/viewReviews.php';
  }
?>
